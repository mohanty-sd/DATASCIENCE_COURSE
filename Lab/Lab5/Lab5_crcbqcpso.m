function outResults = Lab5_crcbqcpso(inParams,psoParams,nRuns)
%Regression of quadratic chirp using PSO
%O = CRCBQCPPSO(I,P,N)
%I is the input struct with the fields given below.  P is the PSO parameter
%struct. Setting P to [] will invoke default parameters (see CRCBPSO). N
%is
...
fHandle = @(x) Lab5_crcbqcfitfunc(x,inParams);
...
for lpruns = 1:nRuns   
    ...
    normSigSqrd = innerprodpsd(estSig,estSig,inParams.sampFreq,inParams.psdPosFreq);
    estSig = 1*estSig/sqrt(normSigSqrd);    %1是snr, normalization
